...
| Phase 8 | **COMPLETE** | 8/8 | Dashboard: tabbed views (Overview/Activity/Metrics), stat cards with hover effects, Recent activity as Table, Overdue Badge with icon + relative time, heat distribution bar, conversion/active metrics. Item 7 (accessibility audit) completed as Phase 9. Item 8 (mobile responsiveness) completed as Phase 10. Item 6 (QA sweep) completed — Topbar on all pages, sr-only headings, toast fixes, DialogDescription, type error fixes |
...
**Phase 8 Item 6 -- QA Sweep**
- Added `<Topbar>` to all 12 pages that were missing it (follow-ups, analytics, analytics/collections, smart-lists, banned, unsubscribed, promos, settings, change-password, new client, edit client, client detail) for consistent sticky header with sidebar trigger + search shortcut
- Replaced visible `<h1>` headings with `<h1 className="sr-only">` on pages where Topbar now shows the page title, avoiding duplicate headings for accessibility
- Changed `container mx-auto py-6 px-4` to `flex-1 p-4 md:p-6` on all content wrappers for consistent padding within the layout's main element
- `components/client-sidebar.tsx` -- changed `toast()` to `toast.success()`/`toast.error()` for proper Sonner toast variants; added `DialogDescription` to promo matches dialog
- `components/client-provider.tsx` -- added missing `customerId`, `employeeName` fields to `FullClient` interface
- Fixed all pre-existing TypeScript errors: `Calendar` `selected` prop `Date | null` → `?? undefined` (5 instances across edit page, new page, edit-client-dialog, follow-up-form); `Set` return types cast to `string[]` (interests-tab, tags-tab); `useState(null)` typed as `any` (new client duplicateClient, edit-client-dialog duplicateClient); `useState([])` typed with explicit generics (edit-client-dialog employees, productsOfInterest, tags)
- Build now passes with zero type errors (was 6+ pre-existing errors before)
